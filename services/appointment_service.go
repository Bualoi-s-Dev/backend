...
func (s *AppointmentService) GetAllAppointment(ctx context.Context, id string) ([]models.Appointment, error) {
...
func (s *AppointmentService) UpdateAppointment(ctx context.Context, id string, appointment *models.Appointment) error {
	if appointment.StartTime.Before(time.Now()) {
		return errors.New("start time must be in the future")
	}
	if appointment.StartTime.After(appointment.EndTime) {
